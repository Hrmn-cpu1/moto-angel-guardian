-- Duration-only journeys must not invent a measured distance.
alter table public.trips add column if not exists distance_measured boolean not null default true;
...
as $$
  with t as (
    select
      ...
      case when tr.distance_measured then round(tr.distance_km, 1)::text || ' km em ' else 'Distância não medida · ' end ||
      ...
      jsonb_build_object(
        'distance', case when tr.distance_measured then tr.distance_km else null end,
